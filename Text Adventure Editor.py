...
def getDefaultGame():
    """ creates the simplest possible default game """
...
def saveGame(game):
    """ uses JSON module to print out game and save it
        to a file """
    fileOut = open("game.json", "w")
    print(jso.dumps(game, indents=2))
...
def editField(prompt, currentVal):
    """ prompts for a field
        if user presses enter key, keep current value """
    
    newVal = input("{prompt} ({currentva}): ")
    if newval == "":
        newVal = currentVal
